...
function Editor(props){
    const [selectedNote, setSelectedNote] = useState({
      ...
      function handleClick(className) {
        ...
        if (editorTextarea) {
          ...
          const selectedText = editorTextarea.value.substring(selectionStart, selectionEnd);
      
          if (className === "highlight" && selectedText) {
            const newText =
              editorTextarea.value.substring(0, selectionStart) +
              `<span style="background-color: yellow">${selectedText}</span>` +
              editorTextarea.value.substring(selectionEnd);
    ...
    return(
        ...
        <div className="editor-container">
            <button className="highlight" onClick={() => handleClick("highlight")}>
